...
const productNames = {
  'aguacate-hass':'Hass avocado','tomate-chonto':'Fresh tomato','manzana-roja':'Red apples','banano-criollo':'Local bananas','pechuga-pollo':'Chicken breast','carne-molida':'Premium ground beef','leche-entera':'Whole milk','yogurt-griego':'Greek yogurt','pan-campesino':'Country bread','croissant-mantequilla':'Butter croissant','cafe-molido':'Ground coffee','jugo-naranja':'Orange juice','arroz-premium':'Premium rice','pasta-spaghetti':'Spaghetti pasta','papas-naturales':'Potato chips','galletas-avena':'Oat cookies','verduras-mixtas':'Frozen mixed vegetables','helado-vainilla':'Vanilla ice cream','detergente-liquido':'Liquid detergent','jabon-manos':'Hand soap','zanahoria':'Fresh carrots','espinaca':'Baby spinach','queso-campesino':'Farmers cheese','huevos-aa':'Grade AA eggs','atun-agua':'Tuna in water','agua-sin-gas':'Still water','pizza-vegetariana':'Vegetarian pizza','esponja-cocina':'Kitchen sponge','shampoo':'Repair shampoo','chocolate':'Milk chocolate','cebolla-cabezona':'Yellow onion','ajo':'Fresh garlic','pimenton-rojo':'Red bell pepper','cilantro':'Fresh cilantro','papa-pastusa':'Potatoes','limon-tahiti':'Tahiti lime','muslos-pollo':'Chicken thighs','salmon':'Salmon fillet','tortillas':'Corn tortillas','frijol-rojo':'Red beans','lentejas':'Selected lentils','aceite-oliva':'Extra virgin olive oil','salsa-tomate':'Italian tomato sauce','crema-leche':'Cooking cream','mozzarella':'Mozzarella cheese','mantequilla':'Salted butter','avena':'Rolled oats','miel':'Pure honey','granola':'Berry granola','nueces':'Premium walnuts','harina-trigo':'Wheat flour','azucar':'Refined sugar','cacao':'Premium cocoa powder','chispas-chocolate':'Chocolate chips','vainilla':'Vanilla extract','polvo-hornear':'Baking powder','fresas':'Fresh strawberries','arandanos':'Premium blueberries','banano-maduro':'Ripe bananas','gelatina':'Unflavored gelatin','leche-condensada':'Condensed milk','arequipe':'Milk caramel spread','helado-chocolate':'Chocolate ice cream','galletas-vainilla':'Vanilla cookies'
};
...
].map(([sku,name,brand,category,price,stock,unit,emoji], index) => { const englishName = productNames[sku] || name; return { sku, name: englishName, brand, category, price, stock, unit, emoji, available: stock > 0, description: `${englishName} selected for MarkECIA.`, rating: Number((4.55 + (index % 5) * 0.1).toFixed(1)), reviews: 80 + index * 17, updatedAt: new Date() }; });

async function seedDatabase() {
  const buyerPassword = process.env.DEMO_BUYER_PASSWORD;
  const adminPassword = process.env.DEMO_ADMIN_PASSWORD;
  if (!buyerPassword || !adminPassword) throw new Error('DEMO_BUYER_PASSWORD and DEMO_ADMIN_PASSWORD are required to seed demo accounts');
  ...
  for (const [email, name, role, password] of [['buyer@markecia.demo','Demo Buyer','BUYER',buyerPassword], ['admin@markecia.demo','Demo Administrator','ADMIN',adminPassword]]) {
  ...
  await orders.updateOne({ reference: 'MK-DEMO-001' }, { $set: { userId: buyer._id, status: 'DELIVERED', items: sample.map((p) => ({ productId: p._id, name: p.name, quantity: 1, price: p.price })), total: sample.reduce((total, p) => total + p.price, 0), updatedAt: new Date() }, $setOnInsert: { reference: 'MK-DEMO-001', createdAt: new Date() } }, { upsert: true });
}
